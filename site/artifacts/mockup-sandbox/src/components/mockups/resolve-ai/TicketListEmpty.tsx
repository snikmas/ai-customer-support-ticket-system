    ...
    <div className="flex h-screen w-full bg-[#f4f5f7] font-sans text-slate-900 overflow-hidden">
      ...
      <aside className="w-[220px] bg-white border-r border-[#e2e4e7] flex flex-col shrink-0 h-full z-10">
        ...
        <div className="flex-1 overflow-y-auto py-4 flex flex-col gap-1 px-3">
          <NavItem icon={<Inbox className="w-4 h-4" />} label="All Tickets" active count={0} />
          <NavItem icon={<Layers className="w-4 h-4" />} label="My Tickets" count={0} />
          <NavItem icon={<Clock className="w-4 h-4" />} label="Unassigned" count={0} />
          <NavItem icon={<AlertTriangle className="w-4 h-4 text-red-500" />} label="Overdue" count={0} />
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden">
        ...
        <main className="flex-1 overflow-auto p-8 custom-scrollbar">
          <div className="max-w-[1200px] mx-auto">
            ...
            {/* Empty State */}
            <div className="bg-white border border-[#e2e4e7] rounded-xl shadow-sm overflow-hidden min-h-[400px] flex flex-col items-center justify-center text-center">
              <Inbox className="w-12 h-12 text-slate-300 mb-4" />
              <h3 className="text-[16px] font-medium text-slate-700 mb-1">No tickets found</h3>
              <p className="text-[14px] text-slate-500 mb-6">Try adjusting your filters or create a new ticket.</p>
              <Button className="bg-blue-600 hover:bg-blue-700 text-white shadow-none text-sm font-medium gap-2 px-4 rounded-md">
                <Plus className="w-4 h-4" /> Create ticket
              </Button>
            </div>
